...
exec /opt/backup/backup-daemon \
  ...
  --backup-cmd "/opt/backup/main.py backup -f {{.data_folder}} -d {{.dbs}}" \
  --restore-cmd "/opt/backup/main.py restore -f {{.data_folder}} -d {{.dbs}} -m {{.dbmap}} -restore_roles {{.restore_roles}}" \
